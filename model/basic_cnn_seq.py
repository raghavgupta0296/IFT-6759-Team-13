...
class MyModel(Model):
    def __init__(self):
        ...
        self.conv1 = Conv3D(
            32, kernel_size=(
                1, 3, 3), input_shape=(
                5, 70, 70, 5), kernel_initializer='he_uniform')
        self.batchnorm1 = BatchNormalization()
        self.relu = ReLU()
        self.maxpool = MaxPool3D((1, 2, 2))
        self.conv2 = Conv3D(
            64, kernel_size=(
                1, 3, 3), kernel_initializer='he_uniform')
        self.batchnorm2 = BatchNormalization()
        self.conv3 = Conv3D(128, kernel_size=(1, 3, 3),
                            kernel_initializer='he_uniform')
        self.batchnorm3 = BatchNormalization()
        self.conv4 = Conv3D(256, kernel_size=(1, 3, 3),
                            kernel_initializer='he_uniform')
        self.batchnorm4 = BatchNormalization()
        self.conv5 = Conv3D(512, kernel_size=(1, 5, 5),
                            kernel_initializer='he_uniform')
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        sdl_train = SequenceDataLoaderMemChunks(
            args,
            catalog_train).map(preprocess).prefetch(
            tf.data.experimental.AUTOTUNE).batch(
            args.batch_size)
        sdl_val = SequenceDataLoaderMemChunks(
            args,
            catalog_val).map(preprocess).prefetch(
            tf.data.experimental.AUTOTUNE).batch(
            args.batch_size)
